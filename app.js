...
const DEFAULT_DATA = {
  ...
  city: 'London',
  groups: {
    dev: { icon: 'bi-code-slash', bookmarks: [
    ...
    tools: { icon: 'bi-wrench', bookmarks: [
...
function saveTheme(accent) {
  localStorage.setItem(THEME_KEY, JSON.stringify({ accent }));
}

function resetTheme() {
  document.documentElement.style.setProperty('--accent', DEFAULT_ACCENT);
  document.getElementById('accentColorPicker').value = DEFAULT_ACCENT;
    ...
    if (saved) {
      ...
      for (const g in data.groups) {
        ...
        if (Array.isArray(group)) {
          data.groups[g] = { icon: 'bi-folder', bookmarks: group };
        } else {
          if (!group.icon) group.icon = 'bi-folder';
...
function renderTabs() {
  ...
  tabsEl.innerHTML = groups.map(g => `
    <button class="tab ${g === data.activeTab ? 'active' : ''}" data-tab="${g}">
      <i class="bi ${data.groups[g].icon || 'bi-folder'}"></i>
      <span class="tab-name">${g}</span>
      <span class="edit" data-edit-group="${g}">✎</span>
  ...
  tabsEl.querySelectorAll('.tab').forEach(tab => {
    tab.addEventListener('click', e => {
      if (e.target.classList.contains('remove')) {
        ...
        e.stopPropagation();
        openEditGroupModal(e.target.dataset.editGroup);
      } else {
...
function renderGroupIconPicker(filter = '') {
  ...
  const filtered = ICONS.filter(i => i.toLowerCase().includes(filter.toLowerCase()));
  picker.innerHTML = filtered.map(icon => `
    <span class="icon-option ${icon === selectedGroupIcon ? 'selected' : ''}" data-icon="${icon}">
      <i class="bi ${icon}"></i>
    </span>
  `).join('');
  ...
  picker.querySelectorAll('.icon-option').forEach(opt => {
    opt.addEventListener('click', () => {
      selectedGroupIcon = opt.dataset.icon;
      document.getElementById('groupIconPreview').className = `bi ${selectedGroupIcon}`;
      renderGroupIconPicker(document.getElementById('groupIconSearch').value);
    });
  });
}
...
function saveGroup() {
  ...
  if (editingGroupName) {
    if (name !== editingGroupName) {
      data.groups[name] = data.groups[editingGroupName];
      delete data.groups[editingGroupName];
    }
    data.groups[name].icon = selectedGroupIcon;
    data.activeTab = name;
  } else {
    if (data.groups[name]) return;
    data.groups[name] = { icon: selectedGroupIcon, bookmarks: [] };
  }
...
function saveBookmark() {
  ...
  if (name && url) {
    if (!data.groups[data.activeTab]) data.groups[data.activeTab] = { icon: "bi-folder", bookmarks: [] };
    if (editingBookmarkIndex !== null) {
      data.groups[data.activeTab].bookmarks[editingBookmarkIndex] = { name, url, icon };
    } else {
      data.groups[data.activeTab].bookmarks.push({ name, url, icon });
    }
    saveData();
    renderBookmarks();
...
function search(query) {
  ...
  const hasTld = /\.[a-z]{2,}$/i;
  
  if (urlPattern.test(query) && hasTld.test(query)) {
...
document.getElementById('toggleGroupIconPicker').addEventListener('click', () => {
  const picker = document.getElementById('groupIconPicker');
  const search = document.getElementById('groupIconSearch');
  const isVisible = picker.classList.contains('visible');
  if (isVisible) {
    picker.classList.remove('visible');
    search.style.display = 'none';
...
document.getElementById('groupIconSearch').addEventListener('input', e => {
  renderGroupIconPicker(e.target.value);
});
